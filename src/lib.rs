    ...
    (mov $(r $addr: expr ,)+ n $num: expr $(, tmp $tmp:expr)? , tar $target: ident) => {
        ...
        match tmp {
            ...
            Some(t) => {
                ...
                let pre_com = [[3, 5, 0], [4, 4, 0], [4, 4, 1], [3, 6, 0], [3, 6, 1], [4, 5, 0], [3, 7, 0], [3, 7, 1], [4, 6, -1], [4, 6, 0], [5, 5, 0], [5, 5, 1], [5, 5, 2], [4, 7, 0], [4, 7, 1], [5, 6, 0], [5, 6, 1], [4, 8, 0], [4, 8, 1], [5, 7, -1], [5, 7, 0], [6, 6, 0], [6, 6, 1], [6, 6, 2]];
                todo!()
            }
        }
        $target.push_str(&output);
