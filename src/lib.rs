...
mod tests {
    ...
    fn test_mov_num() {
        ...
        bf_asm!(mov ram 0, number 0, tmp 1, target bf_code0, clean_target_ram false, clean_tmp_ram false);
        simplify_bf!(code bf_code0, target bf_code0);
        assert_eq!(bf_code0, "");
        let mut bf_code1 = String::new();
        bf_asm!(mov ram 0, number 14, tmp 1, target bf_code1, clean_target_ram false, clean_tmp_ram false);
        simplify_bf!(code bf_code1, target bf_code1);
        assert_eq!(bf_code1, "++++++++++++++");
        let mut bf_code2 = String::new();
        bf_asm!(mov ram 0, number 23, tmp 1, target bf_code2, clean_target_ram false, clean_tmp_ram false);
